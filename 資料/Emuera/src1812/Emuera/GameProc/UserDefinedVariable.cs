			...
			while (!wc.EOL && (idw = wc.Current as IdentifierWord) != null)
			{
				...
				switch (keyword)
				{
					...
					case "REF":
						...
						if (ret.CharaData)
							throw new CodeEE(keyword + "とCHARDATAキーワードは同時に指定できません", sc);
						if (ret.Global)
							throw new CodeEE(keyword + "とGLOBALキーワードは同時に指定できません", sc);
						if (ret.Save)
							throw new CodeEE(keyword + "とSAVEDATAキーワードは同時に指定できません", sc);
					...
					case "CHARDATA":
						...
						//if (staticDefined)
						//    if (ret.Static)
						//        throw new CodeEE("CHARDATAとSTATICキーワードは同時に指定できません", sc);
						//    else
						//        throw new CodeEE("CHARDATAとDYNAMICキーワードは同時に指定できません", sc);
						//if (ret.Global)
						//    throw new CodeEE("CHARDATAとGLOBALキーワードは同時に指定できません", sc);
						//if (ret.CharaData)
						//    throw new CodeEE(keyword + "キーワードが二重に指定されています", sc);
						//ret.CharaData = true;
						//break;
